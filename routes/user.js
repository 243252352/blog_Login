...
router.put("/:id", checkForAuthenticationCookie("token"), async (req, res) => {
    const { id } = req.params;
    const { fullName, email, password } = req.body;

    // Allow update only if the logged-in user matches the param id
...
// DELETE user by ID (only the user themselves can delete)
